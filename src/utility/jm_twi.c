...
#include <Arduino.h> // jmP: for digitalWrite
...
void twi_init(void)
{
// reset and release twi module... + 2015-08-12
TWCR = (0<<TWINT) | (0<<TWEA) | (0<<TWSTA) | (0<<TWSTO) | (0<<TWEN) | (0<<TWIE); // bits TWWC and 1 are R.
...
//	digitalWrite(SDA, 1);
//	digitalWrite(SCL, 1);
pinMode(SDA, INPUT_PULLUP); // + 2015-08-12
...
//		continue;
//	}
if (twi_state != TWI_READY) return 0; // + 2015-08-14
...
//	}
//digitalWrite(13, HIGH);
uint16_t us0; // + 2015-08-12
uint16_t us1;
us0 = micros();
while(true && (TWI_READY != twi_state))
{
us1 = micros();
//if ((uint16_t) (us1 - us0) > ((1000000L+TWI_FREQ-1)/TWI_FREQ)*(1+8+1+2)*(1+length)) return 0; // 100kBps
//if ((uint16_t) (us1 - us0) > ((1000000L+TWI_FREQ-1)/TWI_FREQ)*(4+8+4)*(1+length+1)) return 0; // 400kBps
//if ( (uint16_t) (us1 - us0) > (100+(1000000L+TWI_FREQ-1)*(4+8+4)*(2+length+2)/TWI_FREQ) ) return 0; // 400kBps
if ( (uint16_t) (us1 - us0) > (twi_readFrom_timeout + 100+(1000000L+TWI_FREQ-1)*(4+8+4)*(2+length+2)/TWI_FREQ) ) return 0; // 400kBps
}
//digitalWrite(13, LOW);
...
//	while(TWI_READY != twi_state){ // - 2015-08-12
//		continue;
//	}
if (twi_state != TWI_READY) return 5; // + 2015-08-12
...
uint16_t us1;
us0 = micros();
while(wait && (TWI_READY != twi_state))
//while(wait && (TWI_MTX == twi_state))
{
us1 = micros();
//if ((uint16_t) (us1 - us0) > ((1000000L+TWI_FREQ-1)/TWI_FREQ)*(1+8+1+2)*(1+length)) return 6; // 100kBps
//if ((uint16_t) (us1 - us0) > ((1000000L+TWI_FREQ-1)/TWI_FREQ)*(4+8+4)*(1+length+1)) return 6; // 400kBps
if ( (uint16_t) (us1 - us0) > (100+(1000000L+TWI_FREQ-1)*(4+8+4)*(2+length+2)/TWI_FREQ) ) return 6; // 400kBps
}
	// wait until twi is ready, become master transmitter
...
//digitalWrite(13, LOW);
		...
		case TW_MT_SLA_NACK:	// 20 - address sent, nack received
...
//digitalWrite(13, LOW);
		...
		case TW_MR_SLA_NACK: // 48 - address sent, nack received
			twi_stop();
			break;
		...
		case TW_SR_DATA_NACK:			 // 88 - data received, returned nack
		case TW_SR_GCALL_DATA_NACK: // 98 - data received generally, returned nack
			// nack back at master
			twi_reply(0);
			break;
